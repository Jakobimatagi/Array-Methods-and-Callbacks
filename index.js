...
function getFinals(data) {
    const finalsOnly = data.filter(function(item){
...
function getWinners(getFinalsCB) {
    ...
    if(getFinalsCB['Home Team Goals'] > getFinalsCB['Away Team Goals']){ 
      winner.push(item["Home Team Name"])
    } else {
        winner.push(item["Away Team Name"])}
        })
    return winner  
  }
...
function getWinnersByYear(data, getYearsCB, getWinnersCB) {
   ...
   const winnerYear = winner.map(function(item, index){
...
function getAverageGoals(data) {
    ...
    return (avgGoals / data.length).toFixed(2);
 }
